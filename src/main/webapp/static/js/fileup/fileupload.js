function fileUpLoad() {
	...
	this.itemnumber = parseInt(Math.random() * 10000000);
}
...
fileUpLoad.prototype.setInitInfo = function() {
	...
	this.wrap = "#uploader" + _this.itemnumber;
	this.createInfo = {
		pick: {
			id: '#filePickerDiv' + _this.itemnumber,
			label: '点击选择图片'
		},
		dnd: '#uploader' + _this.itemnumber + ' .queueList',
		paste: document.body,
		/*		accept: {
					title: 'Images',
...
fileUpLoad.prototype.appenToDiv = function() {
	var _info = this.infotext?this.infotext:'或将文件拖到这里，单次最多可选300张';
	var _this = this;
	var html = $('<div id="uploader' + _this.itemnumber + '" class="fileuploader"><div class="queueList queueList' + _this.itemnumber + '"><div id="dndArea' + _this.itemnumber + '" class="placeholder placeholder' + _this.itemnumber + '">' +
		'<div id="filePickerDiv' + _this.itemnumber + '"></div><p>'+_info+'</p>' +
		'</div></div><div class="statusBar statusBar' + _this.itemnumber + '" style="display:none;"><div class="progress progress' + _this.itemnumber + '">' +
		'<span class="text">0%</span><span class="percentage percentage' + _this.itemnumber + '"></span>' +
		'</div><div class="info info' + _this.itemnumber + '"></div><div class="btns ' + _this.itemnumber + '"><div id="filePicker' + _this.itemnumber + '"></div>' +
		'<div class="uploadBtn uploadBtn' + _this.itemnumber + '">开始上传</div></div></div></div>');
...
fileUpLoad.prototype._init = function() {
	...
	this.$queue.appendTo(this._wrap.find('.queueList' + _this.itemnumber));
	// 状态栏，包括进度和控制按钮
	this.$statusBar = this._wrap.find('.statusBar' + _this.itemnumber);
	// 文件总体选择信息。
	this.$info = this.$statusBar.find('.info' + _this.itemnumber);
	// 上传按钮
	this.$upload = this._wrap.find('.uploadBtn' + _this.itemnumber);
	// 没选择文件之前的内容。
	this.$placeHolder = this._wrap.find('.placeholder' + _this.itemnumber);
	// 总体进度条
	this.$progress = this.$statusBar.find('.progress' + _this.itemnumber).hide();
	// 添加的文件数量
	this.fileCount = 0,
		// 添加的文件总大小
		this.fileSize = 0;
	...
	this.uploader.addButton({
		id: '#filePicker' + _this.itemnumber,
		label: '继续添加'
	});
};
...
fileUpLoad.prototype.onUploadProgress = function() {
	...
	this.uploader.onUploadProgress = function(file, percentage) {
		var $li = $('#' + file.id);
		var $percent = $li.find('.progress' + _this.itemnumber + ' span');
